...
public class WhitelistMatcher {
    private static final Logger log = LogManager.getLogger(WhitelistMatcher.class);
     ...
     * @return true if the IP address is whitelisted, false otherwise.
     */
    public static boolean isWhitelisted(String ipAddress, WhitelistConfig config) {
        // If whitelist is not enabled, no IPs are whitelisted.
        if (!config.isEnabled()) {
            return false;
        }
        ...
        try {
            ...
            // Check against each entry in the whitelist.
            for (String entry : entries) {
                if (matches(address, entry)) {
                    log.debug("Whitelisted connection from {} (matched entry: {})", ipAddress, entry);
                    return true;
                }
            }
     ...
     * @param entry   The whitelist entry (can be single IP or CIDR).
    ...
    private static boolean matches(InetAddress address, String entry) {
        ...
        try {
            ...
            log.warn("Failed to parse whitelist entry: {}", entry, e);
            return false;
        }
    }
